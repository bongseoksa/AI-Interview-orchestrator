...
### GM7 — 지시 채팅 & 회의 — [`07`](07-chat-and-meeting.md) **[완료 2026-08-07]**
- 메신저 입력창 → 라우터(@멘션 / LLM / 되묻기) → 기존 Crew 또는 **임의 에이전트 조합 회의**
- 참석자 수가 곧 모드: 1명=작업, 2명+=회의(서기 Task 자동 추가)
- 회의록·결정사항 초안 → 확인함 승인 → 노션 자식 페이지 2건
- `CrewRunner` 거절 제거 — `ThreadPoolExecutor(1)`이 곧 큐
- **DoD**: 스크립트를 짜지 않고 채팅만으로 새 업무를 지시하고, 회의 결과를 노션에 남긴다 ✅
- GM5(확인함)의 최소 버전을 포함한다 — 파일 기반 큐, sqlite 없음
